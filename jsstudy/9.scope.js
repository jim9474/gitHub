 ...
 * ※Scope(스코프)
 * 
 * 스코프란 식별자(변수명, 함수명...)의 유효 범위(참조범위)
 * JS는 전역(global), 지역(local), 모듈(module) 스코프로 구분
 * 식별자가 어디에서 선언되었는지를 스코프의 유효 범위로 보는 정적스코프(렉시컬스코프)를 가진다.
 ...
 * ES6의 변수는 let, const:전역스코프, function:지역스코프, block:지역스코프, 모듈스코프
 * 
...
// local - function
// 함수 안에서 선언한 식별자는 함수스코프(함수 내부에서만 참조 가능)
function f2() {
...
//지역스코프
function f5() {
    console.log(ex_var);    // ex_var
    console.log(ex_let);    // ex_let
    ex_var = 'f5_ex_var';
    ex_let = 'f5_ex_let';
    // var ex_var;  // 1. 현재라인 주석해제시 결과 예측 // undefined    : 이미 선언되어있어서 호이스팅이 아닌 초기화가 된다.
    // let ex_let;  // 2. 현재라인 주석해제시 결과 예측 // error        : let으로 같은 식별자를 선언 못해서 에러난다.
}
f5();
